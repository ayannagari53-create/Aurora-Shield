...
export class RuleEngineService {
  ...
  public static evaluateCompliance(
    ...
    if (!pName || pName.includes('Not detected')) missingMandatory.push('Product Generic Name');
    if (!mrp || mrp.includes('Not detected')) missingMandatory.push('Maximum Retail Price (MRP)');
    if (!country || country.includes('Not detected')) missingMandatory.push('Country of Origin');
    if (!batch || batch.includes('Not detected')) missingMandatory.push('Batch / Lot Number');
    ...
    const overallScore = totalRules === 0 ? 0 : Math.max(
      15,
      Math.min(100, Math.round((passedCount * 100 + needsVerificationCount * 50) / totalRules))
    );
