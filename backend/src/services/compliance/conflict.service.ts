import { DetectedConflict, LabelAngle } from '../../types/compliance.types.js';
...
export class ConflictService {
   ...
   */
  public static detectConflicts(ocrResults: OcrImageResult[]): DetectedConflict[] {
    const conflicts: DetectedConflict[] = [];
    if (!ocrResults || ocrResults.length === 0) return conflicts;

    // Helper maps for attributes per image angle
    const quantityMap: { sourceAngle: LabelAngle; sourceImageId: string; value: string; confidence: number }[] = [];
    const mrpMap: { sourceAngle: LabelAngle; sourceImageId: string; value: string; confidence: number }[] = [];
    const nameMap: { sourceAngle: LabelAngle; sourceImageId: string; value: string; confidence: number }[] = [];
