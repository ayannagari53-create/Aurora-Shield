...
import { OcrImageResult } from '../ocr/ocr.service.js';
...
export class ExtractionService {
  ...
  public static async extractStructuredData(
    ...
    userCategory: string,
    imageDataUrls?: { imageId: string; labelAngle: LabelAngle; dataUrl: string }[]
   ...
   */
  private static async callExternalVisionAi(
    ocrResults: OcrImageResult[],
    imageDataUrls: { imageId: string; labelAngle: LabelAngle; dataUrl: string }[],
    category: string,
    apiKey: string
...
Respond ONLY with a valid JSON object matching this exact structure:
{
   ...
   */
  public static runUniversalProductIntelligence(
    ocrResults: OcrImageResult[],
    userCategory: string
  ): StructuredProductData {
    const allText = ocrResults.map(r => r.rawText).join('\n\n');
    const allLinesWithOrigin: { text: string; angle: LabelAngle; imgId: string; confidence: number }[] = [];
    ...
    const evidence: ProductEvidenceItem[] = [];
    ...
    const searchPattern = (
      pattern: RegExp,
      defaultAngle: LabelAngle = 'BACK',
      defaultImg: string = backResult?.imageId || 'img-1'
    ) => {
      for (const res of ocrResults) {
    ...
    let variantName = '';
    let subcategoryName = '';
    let productNameAngle: LabelAngle = frontResult?.labelAngle || 'FRONT';
    let productNameImg = frontResult?.imageId || 'img-1';
    let productNameConf = 0.0;
    let productNameStatus: ExtractionStatus = 'MISSING';

    let brandAngle: LabelAngle = frontResult?.labelAngle || 'FRONT';
    let brandImg = frontResult?.imageId || 'img-1';
    let brandConf = 0.0;
    let brandStatus: ExtractionStatus = 'MISSING';
    ...
    let foundBrandMatch: string | null = null;
    ...
    if (frontLines.length > 0) {
      const nonAttrLines = frontLines.filter(
        l => !/^(?:net\s*wt|mrp|rs\.|₹|batch|mfg|exp|100%|veg|lic|fssai)/i.test(l) && l.length > 2
      );
      ...
      const firstGoodLine = allLinesWithOrigin.find(
        l => l.text.length > 3 && !/^(?:net|mrp|rs|₹|batch|mfg|exp|fssai)/i.test(l.text)
      );
      if (firstGoodLine) {
    ...
    if (
      lowerAllText.includes('beverage') ||
      ...
      lowerAllText.includes('disinfectant') ||
    ...
    let netQtyStatus: ExtractionStatus = 'MISSING';
    let netQtyConf = 0.0;
    let netQtyAngle: LabelAngle = frontResult?.labelAngle || 'FRONT';
    ...
    let mrpStatus: ExtractionStatus = 'MISSING';
    let mrpConf = 0.0;
    let mrpAngle: LabelAngle = frontResult?.labelAngle || 'FRONT';
    ...
    let ingStatus: ExtractionStatus = 'MISSING';
    let ingConf = 0.0;
    let ingAngle: LabelAngle = ingResult?.labelAngle || 'INGREDIENTS';
    ...
    let allergenStatus: ExtractionStatus = 'MISSING';
    ...
    if (lowerAllText.includes('energy') || lowerAllText.includes('protein') || lowerAllText.includes('carbohydrate') || lowerAllText.includes('fat') || lowerAllText.includes('sugar')) {
      ...
      if (eMatch) nutritionFacts['Energy'] = (nutritionObj.energy = eMatch[1]);
      if (pMatch) nutritionFacts['Protein'] = (nutritionObj.protein = pMatch[1]);
      if (cMatch) nutritionFacts['Carbohydrate'] = (nutritionObj.carbohydrates = cMatch[1]);
      if (totSugMatch) nutritionFacts['Total Sugars'] = (nutritionObj.totalSugars = totSugMatch[1]);
      if (addSugMatch) nutritionFacts['Added Sugars'] = (nutritionObj.addedSugars = addSugMatch[1]);
      if (fMatch) nutritionFacts['Total Fat'] = (nutritionObj.fat = fMatch[1]);
      if (satFMatch) nutritionFacts['Saturated Fat'] = (nutritionObj.saturatedFat = satFMatch[1]);
      if (transFMatch) nutritionFacts['Trans Fat'] = (nutritionObj.transFat = transFMatch[1]);
      if (fiberMatch) nutritionFacts['Fiber'] = (nutritionObj.fiber = fiberMatch[1]);
      if (sodMatch) nutritionFacts['Sodium'] = (nutritionObj.sodium = sodMatch[1]);
      if (servMatch) nutritionFacts['Serving / Reference'] = (nutritionObj.servingSize = servMatch[1]);
    ...
    let mfgStatus: ExtractionStatus = 'MISSING';
    let mfgConf = 0.0;
    ...
    let countryStatus: ExtractionStatus = 'MISSING';
    if (countrySearch) {
    ...
    let batchStatus: ExtractionStatus = 'MISSING';
    if (batchSearch) {
    ...
    let expStatus: ExtractionStatus = 'MISSING';
    if (expSearch) {
    ...
    let fssaiStatus: ExtractionStatus = 'MISSING';
    if (fssaiSearch) {
    ...
    let careStatus: ExtractionStatus = 'MISSING';
    if (careSearch) {
    ...
    if (lowerAllText.includes('warning') || lowerAllText.includes('caution') || lowerAllText.includes('external use only') || lowerAllText.includes('keep out of reach')) {
      ...
      if (warnMatch) warningsList.push(warnMatch[1].trim());
      if (lowerAllText.includes('external use only')) warningsList.push('For external use only.');
      if (lowerAllText.includes('keep out of reach of children')) warningsList.push('Keep out of reach of children.');
    }
    ...
    if (lowerAllText.includes('how to use') || lowerAllText.includes('directions') || lowerAllText.includes('instructions')) {
      ...
      if (usageMatch) usageList.push(usageMatch[1].trim());
    }

    const claimsList: string[] = [];
    if (lowerAllText.includes('100% vegetarian') || lowerAllText.includes('vegetarian')) claimsList.push('100% Vegetarian');
    if (lowerAllText.includes('trans fat free')) claimsList.push('Trans Fat Free');
    if (lowerAllText.includes('gluten free')) claimsList.push('Gluten Free');
    if (lowerAllText.includes('no added sugar') || lowerAllText.includes('zero sugar')) claimsList.push('No Added Sugar');
    if (lowerAllText.includes('dermatologically tested')) claimsList.push('Dermatologically Tested');
    if (lowerAllText.includes('paraben free')) claimsList.push('Paraben Free');
    if (lowerAllText.includes('organic')) claimsList.push('Organic Certified');
    ...
    const labelInformation: LabelInformation = {
    ...
    return {
      ...
      claims: claimsList,
      productInsights: this.generateProductInsights(
        productName,
        brandName,
        detectedCategory,
        subcategoryName,
   ...
   */
  public static generateProductInsights(
    productName: string,
    brand: string,
    category: string,
    subcategory: string,
    ...
    const uses: ProductInsightItem[] = [];
    const benefits: ProductInsightItem[] = [];
    const warningItems: ProductInsightItem[] = [];
    const disadvantages: ProductInsightItem[] = [];
    const limitations: ProductInsightItem[] = [];
  ...
  private static normalizeAiResponse(
    ...
    const allText = ocrResults.map(r => r.rawText).join('\n\n');
    const computedInsights = this.generateProductInsights(
      pName,
      brand,
      aiData.productIdentity?.category || category || 'Food & Beverages',
      aiData.productIdentity?.subcategory || '',
      Array.isArray(aiData.ingredients) ? aiData.ingredients : [],
      aiData.nutrition || {},
      Array.isArray(aiData.allergens) ? aiData.allergens : [],
      Array.isArray(aiData.warnings) ? aiData.warnings : [],
      Array.isArray(aiData.claims) ? aiData.claims : [],
      Array.isArray(aiData.storageInstructions) ? aiData.storageInstructions.join(', ') : 'Store in a cool, dry place.',
      allText
    );

    const productInsights: ProductInsights = {
