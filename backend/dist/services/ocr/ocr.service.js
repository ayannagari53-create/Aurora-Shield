...
export class OcrService {
     ...
     */
    static extractTextFromSvg(svgContent) {
     ...
     */
    static async processImage(imageId, labelAngle, imageInput, customText) {
        ...
        try {
            ...
            if (isSvg && svgText) {
                ...
                if (svgLines.length > 0) {
                    const lines = svgLines.map((lineText, idx) => ({
                        text: lineText,
                        confidence: 0.98,
                        boundingBox: { x: 10, y: 15 + idx * 30, width: Math.min(800, lineText.length * 10), height: 25 }
                    }));
