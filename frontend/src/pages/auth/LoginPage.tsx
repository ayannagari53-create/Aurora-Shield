...
export const LoginPage: React.FC = () => {
  ...
  // Quick Demo Access for Hackathon Judges & Evaluators
  const handleQuickDemoAccess = async () => {
    setError(null);
    setLoading(true);
    try {
      // Create or sign in judge demo account
      const demoEmail = 'judge.sih2026@aurorashield.ai';
      const demoPass = 'SIH2026@JudgeSecure!';
      try {
        await signIn(demoEmail, demoPass);
      } catch {
        // If not created yet in local storage/supabase, auto-register
        await signUp(demoEmail, demoPass, 'SIH Hackathon Evaluator');
      }
      navigate('/dashboard', { replace: true });
    } catch (err: any) {
      setError(err.message || 'Unable to start demo evaluator session');
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <div className="relative my-5">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-slate-950 px-2 text-slate-400 font-mono text-[10px]">
                HACKATHON QUICK EVALUATION
              </span>
            </div>
          </div>

          {/* Quick Demo Login Button for Judges */}
          <button
            type="button"
            onClick={handleQuickDemoAccess}
            disabled={loading}
            className="w-full py-2.5 px-4 rounded-lg font-medium text-xs border border-purple-500/40 bg-purple-950/20 hover:bg-purple-900/30 text-purple-200 transition-all flex items-center justify-center space-x-2 shadow-[0_0_15px_rgba(138,43,226,0.15)] cursor-pointer"
          >
